...
//   f << dot(phi_right) == ((v*sin(theta))/x - ((y*(v*cos(theta)))/pow(x,2)))/(1 + pow(y/x,2))/y - atan(y/x)*(v*sin(theta))/pow(y,2); // Phi over y
...
// 	mpc.set( LEVENBERG_MARQUARDT,         1.0e-4          );
  ...
  if (mpc.exportCode( "DD_export" ) != SUCCESSFUL_RETURN)
		exit( EXIT_FAILURE );
