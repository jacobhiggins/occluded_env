...
classdef mapA3 < map
    ...
    methods
        function setParams(obj)
           ...
           obj.set_wypt_base();
           % Set xlim and ylim
           obj.plt_xlim = [-0.5,obj.hls(2)];
           obj.plt_ylim = [0,obj.hls(1)+0.5];
           % Set corner
           obj.set_corners();
...
%             M3 = -1*eye(2);
            M4 = [0,1;1,0];
            obj.Ms_mpc = {eye(2)};
            obj.walls_mpc = [-1*obj.hws(1)+0.01];
            for i = 1:length(obj.obss)
                obs = obj.obss{i};
...
%             obj.Ms_mpc = cat(2,obj.Ms_mpc,eye(2));
        ...
        function set_patches2d(obj)
           obj.patches.dim.x = 20;
           obj.patches.dim.y = 5;
           obj.patches.start.x = 0;
           obj.patches.start.y = obj.hls(1)-obj.hws(2);
           obj.patches.end.x = obj.hls(2);
           obj.patches.end.y = obj.hls(1);
           obj.patches.probs = 0.5*ones(obj.patches.dim.x,obj.patches.dim.y);
