classdef UGV < handle
   properties
      ...
      MPCoutput = struc('x',[],'u',[]);
      large_num = 100000;
      ku = struct("area",[0.0],"poly",[]);
      kus = struct("areas",[0.0],"polys",[]);
      debug = struct("xrs",[],...
          "vxs",[],...
          "vys",[],...
          "x_accs",[],...
   ...
   methods
       function setParams(obj,map)
           ...
           obj.ax = 0;
           obj.ay = 0;
           obj.xc_mpc_r = map.portion1.corners_r(1,1);
           obj.yc_mpc_r = map.portion1.corners_r(1,2);
           obj.xc_mpc_l = -1*obj.large_num;
           obj.yc_mpc_l = obj.yc_mpc_r;
           obj.xc_wp = map.portion1.corners_r(1,1);
           obj.yc_wp = map.portion1.corners_r(1,2);
           obj.last_sec = false;
           obj.wypt.x = obj.x;
       ...
       function getcorner_MPC(obj,map)
          persistent curr_rc;
          persistent curr_lc;
          if isempty(curr_rc) || obj.t < 0.001 || map.switch_portion
              curr_rc = 1;
              curr_lc = 1;
              if map.switch_portion
                  obj.xc_mpc_r = map.portion2.corners_r(1,1);
                  obj.yc_mpc_r = map.portion2.corners_r(1,2);
              end
          end
          
          if map.current_portion==1
              portion = map.portion1;
          else
              portion = map.portion2;
          end
          
          M = portion.Ms_mpc{curr_rc};
          [~,dely_rc] = c2u(obj.x,obj.y,obj.xc_mpc_r,obj.yc_mpc_r,M);
          [~,dely_lc] = c2u(obj.x,obj.y,obj.xc_mpc_l,obj.yc_mpc_l,M);
          if dely_rc > 0
              
              curr_rc = min(curr_rc + 1,size(portion.corners_r,1));
              obj.rc_active = false;
              obj.safe = false;
          end
          
          
          obj.xc_mpc_r = portion.corners_r(curr_rc,1);
          obj.yc_mpc_r = portion.corners_r(curr_rc,2);
          obj.xc_mpc_l = portion.corners_l(curr_lc,1);
          obj.yc_mpc_l = portion.corners_l(curr_lc,2);
          ...
          if ~map.isVisible(corner_r,obj)
              ...
              if curr_rc < size(portion.corners_r,1)
                  obj.rc_active = true;
              end
          end
          if dely_lc>0 && map.isVisible(corner_l,obj)
              curr_lc =  min(curr_lc + 1,size(portion.corners_l,1));
              obj.lc_active = false;
          elseif map.isVisible(corner_l,obj)%dely_lc > -obj.maxRad
              obj.lc_active = true;
          end
          obj.M_mpc = portion.Ms_mpc{curr_rc};
          obj.current_owall = portion.wall_mpc(curr_rc);
          obj.next_owall = portion.wall_mpc(min(curr_rc+1,length(portion.wall_mpc)));
       end
       function getcorner_WP(obj,map)
           persistent curr_c; % current corner index
           persistent next_c;
           persistent M;
           if isempty(curr_c) || obj.t < 0.001 || map.switch_portion
               curr_c = 1;
               next_c = 2;
               
               if map.switch_portion
                   obj.xc_wp = map.portion2.corners_r(1,1);
                   obj.yc_wp = map.portion2.corners_r(1,2);
                   M = map.portion2.Ms_mpc{1};
               else
                   M = map.portion1.Ms_mpc{1};
               end
           end
           ...
           curr_c = min(curr_c,size(portion.corners_r,1));
           next_c = min(next_c,size(portion.corners_r,1));
           xc1 = portion.corners_r(curr_c,1);
           yc1 = portion.corners_r(curr_c,2);
           xc2 = portion.corners_r(next_c,1);
           yc2 = portion.corners_r(next_c,2);
           avoid = portion.corners_r(curr_c,3);
...
%                next_c = next_c + 1;
%            end
           [x,y] = c2u(obj.x,obj.y,xc1,yc1,M);
           if y > 0
               curr_c = curr_c + 1;
               next_c = next_c + 1;
           end
           try
               obj.xc_wp = portion.corners_r(curr_c,1);
               obj.yc_wp = portion.corners_r(curr_c,2);
               M = portion.Ms_mpc{curr_c};
           catch
...
%            flip = corners_r(curr_c,3);
           ...
           if obj.current_sec == size(portion.corners_r,1)
               obj.last_sec = true;
           end
       ...
       function get_wypt(obj,map)
           ...
           if isempty(dists) || obj.t < 0.001 || map.switch_portion
               vecs = [];
               dists = [];
               c_base = 1;
               for i = 1:length(portion.wypt_bases)-1
                   wypta = portion.wypt_bases(i,:);
                   wyptb = portion.wypt_bases(i+1,:);
                   vec = wyptb - wypta;
                   vec = vec/norm(vec,2);
                   dists = [dists;norm(wypta-wyptb,2)];
                   vecs = [vecs;vec];
               end
               map.switch_portion = false;
           end
           
           if norm([obj.wypt.x-portion.wypt_bases(end,1),obj.wypt.y-portion.wypt_bases(end,2)]) < 0.5
              return; 
           end
           
           c_base = min(length(dists),c_base); % current waypoint index for circle
           wpb = portion.wypt_bases(c_base,:); % current waypoint index for radius
           next_wpb = portion.wypt_bases(min(c_base+1,size(portion.wypt_bases,1)),:); % next waypoint for circle
           next_next_wpb = portion.wypt_bases(min(c_base+2,size(portion.wypt_bases,1)),:); % next next waypoint for circle
           ...
           n = length(portion.wypt_bases);
           xm2 = portion.wypt_bases(min(obj.current_sec+1,n),1); % ALSO CHANGED
           ym2 = portion.wypt_bases(min(obj.current_sec+1,n),2);
           xm3 = portion.wypt_bases(min(obj.current_sec+2,n),1);
           ym3 = portion.wypt_bases(min(obj.current_sec+2,n),2);
%            xm2 = portion.wypt_bases(min(c_base+1,n),1); % ALSO CHANGED
%            ym2 = portion.wypt_bases(min(c_base+1,n),2);
%            xm3 = portion.wypt_bases(min(c_base+2,n),1);
%            ym3 = portion.wypt_bases(min(c_base+2,n),2);
           theta2 = atan2(ym3-ym2,xm3-xm2);
           
           % Set radius using appropriate waypoints
           obj.set_radius(xm2,ym2,theta2,1);
           obj.r = 0.98*obj.r; % HARD CODED FIX FOR BUG
...
%            obj.wypt.y = obj.y + del_vec(2);
       ...
       function shift_wypt(obj,map) % NEW 09/23/2020
           ...
           if obj.current_sec>=4 && obj.portion_1
              obj.safe = true;
              return;
           end
           %            if ~obj.safe
           % Get next closest point on model trajectory
           portion = map.portions{map.current_portion};
           [~,I] = min(vecnorm([portion.model_traj.points(:,1),portion.model_traj.points(:,2)]-[obj.wypt.x,obj.wypt.y],2,2));
           test_point.x = portion.model_traj.points(I,1);
           test_point.y = portion.model_traj.points(I,2);
       ...
       function dist = expected_dist2coll(obj,map,test_point)
           ...
           for i = 1:num_points
               ...
               % Get index of probability that is closest to that position
%                shifted_centers = abs(map.patches.centers - [x_probe;y_probe]);
               shifted_centers.x = abs(map.patches.centers.meshX - x_probe);
               shifted_centers.y = abs(map.patches.centers.meshY - y_probe);
               is = logical(shifted_centers.x <= map.patches.width/2).*logical(shifted_centers.y <= map.patches.height/2);
               prob = map.patches.probs(logical(is'));
               if ~isempty(prob)
                   prob = prob(1);
               else
...
%            upper_bound = 0;
           y_offset = 0;
%            obj.safe = true; %%%% CHANGE LATER
           if obj.rc_active
               if obj.safe
                   perc_r_weight = 0.00000000000005; %10000000;
                   x_weight = 80;
                   upper_bound = 100;
                   y_offset = -10;
                   var_r = 0;
...
%               perc_r_weight = 0.0000005;
%               x_weight = 80;
%               perc_r_weight = 4000;
%               x_weight = 2;
           else
              perc_r_weight = 0.0000000005;
              x_weight = 50;
           end
           ...
           obj.MPCinput.x0 = [xr,yr,vx,vy,var_r,left_bound,right_bound,upper_bound,m_r_inv,0];
...
%                m_l_inv*ones(obj.N+1,1) ...
           ...
           obj.MPCinput.y = [xg*ones(obj.N,1), yg*ones(obj.N,1), var_r*ones(obj.N,1),0*ones(obj.N,1),0*ones(obj.N,1),0*ones(obj.N,1)];
           obj.MPCinput.yN = [xg yg var_des];
           
           % x, y, perception right, perception left, ax, ay, epsilon
           A = diag([x_weight 50 perc_r_weight 25 25 500000]);
...
%            end
           ...
           obj.MPCinput.W = repmat(A,obj.N,1);
           obj.MPCinput.WN = diag([A(1,1) A(2,2) A(4,4)]);
...
%           end
       ...
       function rec_data(obj)
           ...
           obj.rs = [obj.rs obj.r];
           obj.ts = [obj.ts obj.t];
           obj.kus.areas = [obj.kus.areas obj.ku.area];
%            obj.LOSs = [obj.LOSs obj.r];
       ...
       function check_wait(obj,map)
          persistent t;
          if norm([map.portions{map.current_portion}.wypt_bases(end,1)-obj.x,map.portions{map.current_portion}.wypt_bases(end,2)-obj.y])<0.6 && map.current_portion==1
             if isempty(t)
                t = obj.t; 
             elseif (obj.t - t) > 0.5
                map.current_portion = 2;
                map.switch_portion = true;
                map.update_traj();
                obj.portion_1 = false;
             end
          end
