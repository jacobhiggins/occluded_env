...
classdef map < handle
    properties
        ...
        vid
        rec_vid = true;
%         title_string = "HIGH EXPECTED TRAFFIC"
        title_string = "NO EXPECTED TRAFFIC"
    end
    methods %(Abstract)
...
%                 end
            ...
            % Robot (point mass)
            obj.plt_p = plot(p.x,p.y,'ro','MarkerFaceColor','blue','DisplayName','UGV','MarkerSize',5);
            % Waypoint
            obj.plt_wypt = plot(p.x,p.y+p.maxRad,'bx','DisplayName','Waypoint',"LineWidth",3,'MarkerSize',3);
            % Model Trajectory
            obj.plt_traj = plot(obj.portion1.wypt_bases(:,1),obj.portion1.wypt_bases(:,2),'r--','DisplayName','Model Trajectory',...
                "LineWidth",2);
            % Trail of robot
            ...
            if obj.show_FOV
               ...
               obj.plt_FOV = plot(FOV_shape.x,FOV_shape.y,"--",...
                   "DisplayName","FOV",...
                   "Color",[1, 0.47, 0.20]	,...
                   "LineWidth",2);
...
%                 obj.plts = [obj.plts obj.plt_cmd];
            ...
            if obj.show_cornerMPC
                obj.plt_corner_mpc_r = plot([p.x,p.xc_mpc_r],[p.y,p.yc_mpc_r],'g-',"Displayname",'Occluding Corner',"LineWidth",2);
...
%                 obj.plts = [obj.plts obj.plt_corner_mpc_l];
            ...
            % Title
%             title(obj.fig_main.CurrentAxes,sprintf("MPC Motion, Time: %0.1f",p.t));
            title(obj.fig_main.CurrentAxes,obj.title_string);
            % Record video
            if obj.rec_vid
        ...
        function initial_aux(obj,p)
            ...
            ylabel("m");
            yyaxis right;
            obj.plt_ku_area = plot(p.ts,p.kus.areas,'Linewidth',2);
            ylabel("m^2");
            xlabel("Time (s)");
        ...
        function update_plot(obj,p,sim_dt)
            ...
            t_int = t_int + 1;
            if mod(t_int,int16(p.dt/sim_dt)) ~= 0
                return;
            end
...
%                ku = ku_mpc;
               ...
               p.ku = ku;
%                p.ku.areas = [p.kus.areas ku.area];
%                p.ku.polys = [p.ku.polys ku.poly];
            ...
            if obj.show_cornerMPC
                flag = (obj.current_portion==1 && p.current_sec==5) || ...
                    (obj.current_portion==2 && p.current_sec==1);
                if p.rc_active && ~flag
            ...
            obj.plt_ku_area.YData = p.kus.areas;
            % Update title
%             if obj.show_time
...
%             end
        ...
        function postPlots(obj,p)
            ...
            post_data.ts = ts;
            post_data.LOSs = LOSs;
            post_data.kus.areas = p.kus.areas;
            post_data.ku.polys = p.kus.polys;
