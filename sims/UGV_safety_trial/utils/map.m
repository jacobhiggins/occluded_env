...
classdef map < handle
    properties
        ...
        patches = struct("num",10,"xstart",0.0,"xend",0.0,"ybottom",0.0,"ytop",0.0,"width",0.0,"centers",struct("x",[],"y",[],"meshX",[],"meshY",[])); % Patches of uncertainty
        ...
        rec_vid = false;
        rec_gif = true;
        title_string = "UNSAFE WITH PERCEPTION";
%         title_string = "SAFE WITHOUT PERCEPTION";
    end
    methods %(Abstract)
...
%         termcond(obj,p) % returns true/false for map terminal condition
        ...
        function initial_plot(obj,p,people)
            hold on;
            obj.plts = [];
            % Save figure handle (for video)
            obj.fig_main = gcf;
            % Walls
            gray = 0.0*[0.5 0.5 0.5];
            for i = 1:length(obj.walls)
               wall = obj.walls{i};
               patch(wall.x,wall.y,gray);
            end
            patch([obj.hws(1)+0.5 10 10 obj.hws(1)+0.5],[-1 -1 (obj.hls(1)-obj.hws(2)-0.5)*ones(1,2)],ones(1,3))
            % Show patches
            if obj.show_patches
...
%                 end
               for i = 0:(obj.patches.dim.x -1)
                  for j = 0:(obj.patches.dim.y-1)
                      x1 = obj.patches.start.x + i*obj.patches.width;
                      x2 = x1 + obj.patches.width;
                      y1 = obj.patches.start.y + j*obj.patches.height;
                      y2 = y1 + obj.patches.height;
                      plt_patch = patch([x1 x2 x2 x1],[y1 y1 y2 y2],obj.patches.probs(i+1)*ones(1,3),"EdgeColor","none");
                      obj.plt_patches{i+1,j+1} = plt_patch;
                  end
               end
            end
            if obj.show_people
               obj.plt_people = scatter(people.positions.x,people.positions.y,...
               ".",...
               "DisplayName","Simulated Dynamic Object");
               obj.plts = [obj.plts obj.plt_people];
            end
            % Robot (point mass)
            obj.plt_p = plot(p.x,p.y,'ro','MarkerFaceColor','blue','DisplayName','Robot',...
                "MarkerSize",10);
            % Model Trajectory
            obj.plt_traj = plot(obj.wypt_bases(:,1),obj.wypt_bases(:,2),'r--','DisplayName','Model Trajectory',...
                "LineWidth",2);
            % Waypoint
            obj.plt_wypt = plot(p.x,p.y+p.maxRad,'bx','DisplayName','Waypoint','MarkerSize',3,"LineWidth",3);
            % Trail of robot
            obj.plt_trail = plot(p.trail.x,p.trail.y,':','DisplayName','Actual Trajectory','Color','blue','LineWidth',2);
            % Add plots to running list
            obj.plts = [obj.plts,obj.plt_p,obj.plt_wypt,obj.plt_traj,obj.plt_trail];
            % Show circle
            if obj.show_circ
...
%                obj.plts = [obj.plts obj.plt_circ];
            ...
            if obj.show_knownunknown
               ...
               obj.plt_knownunknown = fill(ku.poly.x,ku.poly.y,'r',"DisplayName","Known Unknown Area",...
                   "FaceAlpha",0.2);
               obj.plts = [obj.plts obj.plt_knownunknown];
            end
            % Show projected motion
            if obj.show_proj
                obj.plt_proj = plot(p.proj_mot.x,p.proj_mot.y,'m-','LineWidth',3,'DisplayName','Projected Path');
...
%                 [p.x,p.xc_mpc_r],[p.y,p.yc_mpc_r]
                obj.plt_corner_mpc_r = plot([0 0],[0 0],'g-',"Displayname",'Occluding Corner',"LineWidth",2);
...
%                 obj.plts = [obj.plts obj.plt_corner_mpc_l];
            ...
            % Show Legend
            if obj.show_legend
               lg = legend(obj.plts,'Location','southeast'); 
            end
            % Title
            title(obj.fig_main.CurrentAxes,sprintf("Time: %0.2f",p.t));
            % Record video
            if obj.rec_vid
                obj.vid = VideoWriter('vid','MPEG-4');
                open(obj.vid);
            end
        ...
        function initial_aux(obj,p)
            ...
            ylabel("m^2");
            xlabel("Time (s)");
            legend("LOS","KU Area","Location","southeast");
            
        end
        function update_plot(obj,p,people,sim_dt)
            ...
            if obj.show_patches
                for i = 1:obj.patches.dim.x
                    for j = 1:obj.patches.dim.y
                        plt_patch = obj.plt_patches{i,j};
                        plt_patch.FaceColor = obj.patches.probs(i,j)*ones(1,3);
                    end
                end
            end
            if obj.show_people
               obj.plt_people.XData = people.positions.x;
               obj.plt_people.YData = people.positions.y;
...
%                end
            ...
            % Update title
            title(obj.fig_main.CurrentAxes,sprintf("Time: %0.1f",p.t));
            ...
            if obj.rec_gif && mod(t_int,int16(p.dt/sim_dt)) == 0
                frame = getframe(obj.fig_main);
                im = frame2im(frame);
        ...
        function update_probs2d(obj,p)
            ...
            for i = 1:obj.patches.dim.x
               for j = 1:obj.patches.dim.y
                   ...
                   d = norm([p.x-point_x,p.y-point_y]);
                   theta_point = atan2(point_y-p.y,point_x-p.x);
                   theta_vis = atan2(p.yc_mpc_r - p.y,p.xc_mpc_r - p.x);
                   if d < p.maxRad && ((theta_point > theta_vis) || p.current_sec==2)
                       if rand < p_z0_m0
            ...
            if abs(obj.hls(1)-obj.hws(2)) > p.y
               for t = p.dt:p.dt:max(obj.hls(1)-obj.hws(2)/2-p.y,0)/p.max_v
                   for i = 1:obj.patches.dim.x-1
                      for j = 1:obj.patches.dim.y
                          obj.patches.probs(i,j) = p_move*obj.patches.probs(i+1,j) + p_stay*obj.patches.probs(i,j);
                      end
                   end
               end
            end
