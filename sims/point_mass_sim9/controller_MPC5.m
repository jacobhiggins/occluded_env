function [ax,ay,mpc4Input] = controller_MPC5(p,xg_in,yg_in,vgx_in,vgy_in,axg_in,ayg_in,N,mpc4Input,M,sec,ax_in,ay_in,flip)
    scale = 1;
    left_bound = -29;
    M = [flip 0;0 1]*M;
...
%         yrs ...
    ...
    mpc4Input.y = [xg*ones(N,1) yg*ones(N,1) vgx*ones(N,1) vgy*ones(N,1) 0*ones(N,1) 0*ones(N,1) var_des*ones(N,1) 0*ones(N,1) 0*ones(N,1) 0*ones(N,1)];
...
%     A = eye(6);
    A = diag([10/scale 1/scale 100/scale 100/scale 1/scale 1/scale 1/scale 10/scale 10/scale 100000]); % 5000000
%     A(3,3) = 0.5;
    if sec==3
       A(7,7) = 0.000000000001; 
    end
    mpc4Input.W = repmat(A,N,1);
    mpc4Input.WN = 1*diag([A(1,1) A(2,2) A(9,9)]);
%     mpc3Input.W(2) = 0.000001;
    mpcOutput = acado_solver5( mpc4Input );
