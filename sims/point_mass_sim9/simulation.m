function simulation(record,flags)
    addpath('./utils')
    %% ********* Define Constants ************
...
%         way_pt_base = get_waypoint_base(xs(i),ys(i),hls,hws,obss);
    ...
    xcs = zeros(rs,1); % MPC perception
    ycs = zeros(rs,1); % MPC perception
    xcs_wypt = zeros(rs,1); % Waypoint placement
    ycs_wypt = zeros(rs,1); % waypoint placment
    for i = 1:rs
...
%         plt_goal = plot(xgs(i),ygs(i),'r*','DisplayName','pursuit'); % pursuit
    ...
    for i = 1:rs
        ...
        intersect = set_waypoint(xs(i),ys(i),xcs(i),ycs(i),xm2,ym2,theta2,d);
        xintersect = intersect.x;
        yintersect = intersect.y;
...
%     patch([hls(2)+hws(3)/2,hls(2)+hws(3),hls(2)+hws(3),hls(2)+hws(3)/2],[-hws(2)/2,-hws(2)/2,hls(3),hls(3)],gray);
    ...
    for i = 1:rs
        ...
        qvs{i} = qv;
        plt_line = plot([xs(i),xcs(i)],[ys(i),ycs(i)],'c-',"Displayname",'MPC corner',"LineWidth",2); % Line to corner, for MPC
        plt_line_wp = plot([xs(i),xcs_wypt(i)],[ys(i),ycs_wypt(i)],'g-',"DisplayName",'Waypoint corner',"LineWidth",2); % Line to corner, for waypoint
        plt_lines{i} = plt_line;
        plt_lines_wp{i} = plt_line_wp;
    end
...
%     plot(corners(:,1),corners(:,2),"o");
    ...
    legend([plt,plt_goal2,qv,plt_traj,plt_trail,plt_proj,plt_line,plt_line_wp],'Location','eastoutside'); %********REMOVE WHEN DONE TESTING
...
%         plot([0,200],sysparams.rad_max*ones(2,1));
    ...
    while flag
        ...
        for i = 1:rs
            ...
            % This advances corner when past corner
            % For MPC corner placement
            [xcs(i),ycs(i),M_avoid,flip,c] = setCorner3(xs(i),ys(i),xcs(i),ycs(i),corners,Ms_avoid,i);% c = current corner/waypoint base
            
            % This advances corner when next corner is in LOS
            % For waypoint placement
            [xcs_wypt(i),ycs_wypt(i),~,~,c] = setCorner2(xs(i),ys(i),corners,Ms_avoid);
            ...
            theta2 = atan2(ym3-ym2,xm3-xm2);
            d = norm([ym3-ym2,xm3-xm2],2);
            intersect = set_waypoint(xs(i),ys(i),xcs(i),ycs(i),xm2,ym2,theta2,d); % NOT for setting waypoint anymore
            xintersect = intersect.x; 
            yintersect = intersect.y;
            if sec==3
                M_avoid = eye(2);
...
%             waypoint2 = set_waypoint2(xs(i),ys(i),xcs(i),ycs(i),way_pt_base,prev_wypt,rads(i),i,sysparams.rad_max,c,dist_fracs{i});
            waypoint2 = set_waypoint2(xs(i),ys(i),xcs_wypt(i),ycs_wypt(i),way_pt_base,prev_wypt,rads(i),i,sysparams.rad_max,c,dist_fracs{i});
            xg2 = waypoint2.x;
            yg2 = waypoint2.y;
            ...
            plt.XData = xs(i);
            plt.YData = ys(i);
            [waypoint2,rads(i)] = set_waypoint2(xs(i),ys(i),xcs_wypt(i),ycs_wypt(i),way_pt_base,prev_wypt,rads(i),i,sysparams.rad_max,c,dist_fracs{i});
            sim_rad = sim_rads{i};
            ...
            plt_line_wp = plt_lines_wp{i};
            plt_line_wp.XData = [xs(i),xcs_wypt(i)];
            plt_line_wp.YData = [ys(i),ycs_wypt(i)];
            trail = trails{i};
...
%                 ann.String = sprintf("Cost: %7.0f",mpc6Input.cost);
    ...
    if sysparams.show_vnorm
       ...
       title("LOS Over Time");
       legend;
       % Try plotting velocties vs. y position
       figure(rs+3);
       inds = find(trail(:,2) < hls(1)-hws(2));
       plot(trail(inds,2),vsi(inds));
       xlabel("Y Position (m)");
       ylabel("Velocity (m/s)");
       title("Velocity Over Y Position");
       grid on;
    end
